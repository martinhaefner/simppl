...
struct CallTraits
{
   ...
   typedef typename if_<std::is_reference<T>::value, T, typename std::remove_reference<T>::type&>::type reference_type;
   typedef typename if_<std::is_reference<T>::value, const T, const typename std::remove_reference<T>::type&>::type const_reference_type;   

   /**
    * This typedef could be used for optimized method parameter type dispatching.
    */
   typedef typename if_<std::is_integral<T>::value 
		               || std::is_floating_point<T>::value 
		               || std::is_pointer<T>::value
                     || std::is_reference<T>::value, value_type, const_reference_type>::type param_type;

   typedef typename if_<std::is_reference<T>::value, value_type, reference_type>::type return_type;
